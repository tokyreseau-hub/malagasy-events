...
const [gastroRows,orgas,lieux,events]=await Promise.all(['gastro','organisateurs','lieux','events'].map(query))
// La base conserve les fiches écartées pour permettre une récupération, mais
// seules les adresses dont le lien malagasy est validé doivent être indexables.
const gastro=gastroRows.filter(x=>x.malagasy_verified===true)
...
const staticUrls=['/','/gastronomie','/organisateurs','/sportifs','/tournois','/eglises','/boutiques','/artisanat','/petites-annonces','/diaspora-malgache-france','/guide-france','/a-propos','/contact','/faq']
