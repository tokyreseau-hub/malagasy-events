...
const eventBelongsToOrga = (event, orga) => {
  ...
  const eventName=normalizedOrgaName(event.organizer)
  const orgaName=normalizedOrgaName(orga.name)
  if (!eventName || !orgaName) return false
  if (eventName===orgaName) return true
  const eventAcronym=String(event.organizer||"").trim().match(/^([A-Z0-9]{2,6})\b/)?.[1]
  const orgaAcronym=String(orga.name||"").trim().match(/^([A-Z0-9]{2,6})\b/)?.[1]
  if (eventAcronym && orgaAcronym && eventAcronym===orgaAcronym) return true
  const [shorter,longer]=eventName.length<=orgaName.length?[eventName,orgaName]:[orgaName,eventName]
  // Autorise seulement une vraie forme courte du nom complet ("ASM" / "ASM Paris"),
  // jamais un mot isolé trouvé au milieu comme une ville ("Paris").
...
const dedupeEvents = rows => {
  ...
  ;(rows||[]).forEach(row=>{
    ...
    const previous=merged.get(key)
    merged.set(key,previous
      ? {...row,...previous,image:previous.image||row.image,ticketUrl:previous.ticketUrl||row.ticketUrl,description:previous.description||row.description,mediaUrls:(previous.mediaUrls?.length?previous.mediaUrls:row.mediaUrls)||[]}
      : row)
  })
  return [...merged.values()]
}
...
const trackVisitEvent = (eventName, extra={}) => {
  if(audienceOptedOut()||!trackingSession||!trackingVisitor)return
...
const eventJsonLd = e => ({
  ...
  ...(isVerifiedTicketUrl(e.ticketUrl)?{offers:{"@type":"Offer", url:safeUrl(e.ticketUrl), availability:"https://schema.org/InStock"}}:{}),
...
const initialGastro = [
  {id:1,name:"Ikala Kara",type:"Restaurant",region:"Provence-Alpes-Côte d'Azur",note:"Restaurant malgache & karaoké (ven-sam), quartier La Plaine",fb:"https://www.facebook.com/Ikalakara",insta:"",contact:"www.ikala-kara.fr",city:"Marseille (13)",address:"40 rue Saint-Savournin, 13001 Marseille",phone:"09 80 67 41 54",lat:43.2989,lng:5.3872},
  {id:2,name:"O'Bol d'Or",type:"Restaurant",region:"Île-de-France",note:"Restaurant — spécialités asiatiques & malgaches (soirées Ari Nao)",fb:"https://www.facebook.com/profile.php?id=61558005182008",insta:"",contact:"www.oboldor.com",city:"Vitry-sur-Seine (94)",address:"72 avenue Anatole France, 94400 Vitry-sur-Seine",phone:"",lat:48.7899,lng:2.3938},
  ...
  {id:5,name:"La Gourmandise Malgache",type:"Traiteur",region:"Hauts-de-France",note:"Plats et apéritifs malgaches faits maison",fb:"https://www.facebook.com/profile.php?id=100076189512327",insta:"",contact:"",city:"Lillers (62)",lat:50.5636,lng:2.4819},
  {id:6,name:"Pili Pili Malgache Food",type:"Traiteur",region:"Grand Est",note:"Recettes traditionnelles de Madagascar pour vos événements",fb:"https://www.facebook.com/pilipilimalgachefood",insta:"",contact:"www.pilipilimalgachefood.com",city:"Réguisheim (68)",address:"Grand Rue, 68890 Réguisheim",lat:47.9929,lng:7.3721},
  {id:7,name:"Chez Daben",type:"Traiteur",region:"Île-de-France",note:"Traiteur événementiel malgache",fb:"https://www.facebook.com/profile.php?id=100067003274574",insta:"",contact:"",city:"Paris",address:"Boulevard Victor, Paris",lat:48.8386,lng:2.2872},
  ...
  {id:9,name:"Chez Tiana",type:"Traiteur",region:"Île-de-France",note:"Ravitoto & mofo gasy faits maison, salé et sucré",fb:"https://www.facebook.com/Cheztiana",insta:"",contact:"",city:"Paris 14e",lat:48.8331,lng:2.3264},
  {id:10,name:"Naffees Traiteur",type:"Traiteur",region:"Île-de-France",note:"Spécialités indo-pakistanaises & malgaches, halal",fb:"https://www.facebook.com/traiteurevenementielnaffees",insta:"",contact:"",city:"Argenteuil (95)",lat:48.9472,lng:2.2467},
  {id:11,name:"Nini + Vous",type:"Traiteur",region:"Nouvelle-Aquitaine",note:"Chef à domicile & traiteur franco-malgache — 10 ans de restaurant malgache à Toulouse, formée à Ferrandi",fb:"https://www.facebook.com/profile.php?id=100086724786890",insta:"",contact:"www.ninietvous.fr",city:"Bordeaux",address:"Bordeaux et sa région",phone:"",lat:44.8378,lng:-0.5792},
  ...
  {id:14,name:"Au Soleil de Madagascar",type:"Food truck",region:"Île-de-France",note:"Food truck & traiteur, spécialités malgaches",fb:"https://www.facebook.com/AuSoleildeMadagascar",insta:"",contact:"",city:"Cachan (94)",lat:48.7919,lng:2.3319},
  ...
  {id:16,name:"Cuisine Malgache et d'ailleurs",type:"Communauté",region:"",note:"4 600 membres — groupe de partage de recettes malgaches et du monde",fb:"https://www.facebook.com/EvitraAkohoFaMahaOmby",insta:"",contact:"",city:"",lat:null,lng:null},
  ...
  {id:19,name:"Restaurant Malgache",type:"Restaurant",region:"Provence-Alpes-Côte d'Azur",note:"Cajun et créole, spécialités réunionnaises et malgaches",fb:"https://www.facebook.com/profile.php?id=100068530603400",insta:"",contact:"",city:"Nice",address:"Nice",phone:"",lat:43.7102,lng:7.2620},
  {id:20,name:"Ari Nao",type:"Guide",region:"Île-de-France",note:"La page qui teste et recommande les restos malgaches en Île-de-France",fb:"https://www.facebook.com/itsarinao",insta:"",contact:"",city:"Île-de-France",lat:null,lng:null},
  {id:21,name:"Groupe « Malagasy à Paris »",type:"Communauté",region:"Île-de-France",note:"19 200 membres — recense les bonnes adresses de restos malgaches en Île-de-France",fb:"",insta:"",contact:"",city:"Île-de-France",lat:null,lng:null},
  {id:22,name:"Resto Gasy",type:"Restaurant",region:"Île-de-France",note:"Restaurant traditionnel malagasy fondé par la cheffe Sidonie. Spécialités de l’Île rouge : romazava, ravitoto et gambas au combava et aux baies roses.",fb:"",insta:"",contact:"www.rest-gasy.fr",city:"Paris 6e",address:"29 rue Dauphine, 75006 Paris",phone:"06 41 99 76 06",lat:48.8546,lng:2.3387},
]
...
function NotificationsPage({ user, activeOrga, onAuthRequired, onUnreadChange, onOpenCommunity, onOpenMessages, onOpenEvent }) {
  const [items,setItems] = useState([])
  ...
  const open = async n => {
    ...
    if (n.type==="event_match") onOpenEvent?.(n.data?.event_id)
...
function SubmitEventModal({ user, userProfile, onEventPublished, onClose, events = [] }) {
  const empty = {title:"",date:"",city:"",location:"",address:"",lat:null,lng:null,category:"Soirée",price:"",organizer:"",ticket_url:"",image:"",description:"",submitter_email:user?.email||""}
  ...
  const submit = async () => {
    ...
    if (canDirect) {
      ...
      const payload = {title:f.title,date:f.date,city:f.city,location:f.location,address:f.address||f.location,lat:f.lat,lng:f.lng,category:f.category,price:f.price,organizer:f.organizer||userProfile?.username||"",ticketUrl:safeUrl(f.ticket_url),image:"",description:f.description,mediaUrls:[],featured:true,createdAt:new Date().toISOString(),owner_id:user.id}
      ...
      const {error} = await supabase.from('event_submissions').insert({...f,ticket_url:safeUrl(f.ticket_url),image:"",submitter_id:user?.id||null})
      if (error) {
        const m = error.message||""
...
function GastroPage({ isMobile, gastro = initialGastro, initialSearch="" }) {
  ...
  const mapRef = useRef(null)
  const mapInstance = useRef(null)
  const types = ["Tous","Restaurant","Traiteur","Food truck","Guide","Communauté"]
    ...
    <>
      <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
        ...
        <div style={{padding:"10px 16px 14px",borderTop:"1px solid #f5f5f5",display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          {BILLETTERIE_ACTIVE && isVerifiedTicketUrl(event.ticketUrl)
...
function EntraideSection({ event, user, onAuthRequired, onProfileClick }) {
  ...
  const submit = async e => {
    ...
    if (!error) { resetForm(); await fetchItems() }
    setSaving(false)
  }
...
function EventEditModal({ event, onClose, onSaved }) {
  const [form,setForm] = useState({title:event.title||'',date:event.date||'',city:event.city||'',location:event.location||'',category:event.category||'Soirée',price:event.price||'',organizer:event.organizer||'',ticketUrl:event.ticketUrl||'',image:event.image||'',description:event.description||''})
  const [saving,setSaving] = useState(false)
  ...
  const save = async e => {
    ...
    setSaving(true)
    const payload = {...form,title:form.title.trim(),ticketUrl:safeUrl(form.ticketUrl),image:"",mediaUrls:[]}
...
function EventDetail({ event, onClose, user, onAuthRequired, isAdmin, onUpdated, onProfileClick }) {
  const [showShare,setShowShare]   = useState(false)
  const [showReminder,setReminder] = useState(false)
        ...
        <div style={{background:WHITE,borderRadius:24,width:"100%",maxWidth:680,margin:"auto",boxShadow:"0 24px 80px rgba(0,0,0,0.3)",overflow:"hidden"}}>
          ...
          <div style={{padding:isMobile?"16px":"28px"}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:20}}>
              ...
              {BILLETTERIE_ACTIVE && isVerifiedTicketUrl(event.ticketUrl)
                ? <a href={safeUrl(event.ticketUrl)} target="_blank" rel="noreferrer" style={{background:GREEN,color:WHITE,fontWeight:700,fontSize:13,padding:"10px 18px",borderRadius:12,textDecoration:"none"}}>🎟️ Acheter mes billets</a>
...
function AdminPanel({ events, setEvents, videos, setVideos, gastro, setGastro, orgas, setOrgas, lieux, setLieux, onClose }) {
  ...
  const [trafficVisitorRegistry,setTrafficVisitorRegistry] = useState([])
  ...
  const loadTab = async t => {
    ...
    if (t==="dashboard" || t==="analytics") {
      const [
        ...
        {count:pendingSubmissions},{count:openRights},{count:pendingClaims}
      ] = await Promise.all([
      ...
      setStats({members,posts,cmts,interests,messages,follows,rems,evts:events.length,openReports,pendingSubmissions,openRights,pendingClaims})
    } else if (t==="users") {
      ...
      const [{data,error},{data:registry}]=await Promise.all([
        supabase.from('analytics_events').select('*').gte('created_at',since).order('created_at',{ascending:false}).limit(10000),
        supabase.from('analytics_events').select('visitor_id,created_at').not('visitor_id','is',null).order('created_at',{ascending:true}).limit(10000)
      ])
      if(error) console.warn('Tracking non activé :',error.message)
      setTrafficRows(data||[])
      setTrafficVisitorRegistry(registry||[])
    } else if (t==="sportsdb") {
  ...
  const approveSubmission = async s => {
    const payload = {title:s.title,date:s.date,city:s.city,location:s.location,address:s.address||s.location,lat:s.lat,lng:s.lng,category:s.category,price:s.price,organizer:s.organizer,ticketUrl:s.ticket_url,image:"",description:s.description,mediaUrls:[],createdAt:new Date().toISOString()}
    const {data,error} = await supabase.from('events').insert(payload).select().single()
    if (error) { alert("⚠️ "+error.message); return }
    setEvents(e=>[...e,data])
  ...
  const TAB_GROUPS = [
    ...
    {name:"Communauté",tabs:[{id:"users",l:"👥 Membres"},{id:"posts",l:"📝 Posts"},{id:"comments",l:"💬 Commentaires"},{id:"entraide",l:"🤝 Entraide"},{id:"actus",l:"📣 Actus orgas"}]},
    ...
    {name:"Sécurité & droits",tabs:[{id:"reports",l:"🚩 Signalements"},{id:"rights",l:"🛡️ Demandes RGPD"},{id:"licenses",l:"©️ Autorisations"}]},
      ...
      <div style={{flex:1,overflowY:"auto",padding:24}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          {tab==="traffic" && (()=>{
            ...
            const visitorOrder=new Map();trafficVisitorRegistry.forEach(x=>{if(x.visitor_id&&!visitorOrder.has(x.visitor_id))visitorOrder.set(x.visitor_id,visitorOrder.size+1)})
            const visitorAlias=id=>`Visiteur ${visitorOrder.get(id)||'—'}`
            ...
            return <div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:14,marginBottom:18}}><StatCard n={views.length} l="Pages consultées" emoji="👀"/><StatCard n={sessions} l="Visites du site" emoji="🌐"/><StatCard n={socialVisits.length} l="Visites venues des réseaux" emoji="📲"/><StatCard n={socialClicks.length} l="Clics vers nos réseaux" emoji="↗️"/></div>
...
export default function App() {
  ...
  useEffect(()=>{ // URL + méta par page
    ...
    setMeta(t,d,{index:page!=="notifications" && page!=="community" && page!=="aftermovies"})
  ...
  const loadDb = async () => {
    ...
    if (!ga.error && ga.data?.length) {
      ...
      setGastro(dedupeByName([...ga.data,...missingGastroDefaults]))
    }
    const og = await supabase.from('organisateurs').select('*').order('id')
    if (!og.error && og.data?.length) {
      const verifiedOrgas = og.data.map(item=>String(item.name||"").trim().toLowerCase()==="gas'paname sport"
        ? {...item,followers:"2 800",insta:"https://www.instagram.com/gas_paname_sport/",note:"Communauté sportive malagasy de Paris : Gaspaname Game, Coupe du Monde Gas’Paname, basket et foot inter-lycées de Tana Alumni France."}
        : item)
  ...
  return (
    <div id="mev-app" style={{minHeight:"100vh",background:"#f6f6f6",fontFamily:"system-ui,sans-serif"}}>
      ...
      {page==="notifications" && (
        <NotificationsPage user={user} activeOrga={activeOrga} onAuthRequired={()=>setShowAuth(true)} onUnreadChange={setUnreadNotifications} onOpenCommunity={()=>setPage("community")} onOpenEvent={eventId=>{const event=events.find(e=>String(e.id)===String(eventId));if(event){setPage("home");setSelectedEvent(event)}}} onOpenMessages={n=>{const orgaId=n?.data?.orga_id;const orga=orgas.find(o=>String(o.id)===String(orgaId));if(orga)openMsg({type:'orga',orga});else setShowMessages(true)}}/>
      )}
      ...
      {!["community","notifications","aftermovies"].includes(page) && (
        <footer style={{background:RED,padding:"20px 24px",textAlign:"center"}}>
          ...
          <nav aria-label="Liens utiles Malagasy Events" style={{display:"flex",justifyContent:"center",flexWrap:"wrap",gap:"8px 16px",marginTop:12}}>
            ...
            <a href="/offres" style={{color:WHITE,fontSize:12,fontWeight:700,textDecoration:"none"}}>Nos offres</a>
          </nav>
      ...
      {/* ── MODALS ── */}
      {selectedEvent && (
        <EventDetail event={selectedEvent} onClose={()=>setSelectedEvent(null)} user={user} onAuthRequired={()=>setShowAuth(true)} isAdmin={isAdmin} onProfileClick={(id,name)=>setViewingProfile({id,name})} onUpdated={updated=>{setEvents(list=>list.map(x=>x.id===updated.id?updated:x));setSelectedEvent(updated)}}/>
      )}
