...
export function ClassifiedsAdmin(){
  ...
  const load=useCallback(async()=>{
    ...
    const [adsResult,categoriesResult]=await Promise.all([
      supabase.from("classifieds").select("*,classified_categories(id,name,slug),profiles!classifieds_user_id_fkey(id,username,avatar_url,plan,is_member,created_at)").order("submitted_at",{ascending:false}).limit(300),
